...
CONSTRAINT_RULES.update(
    {
        # Low tier: camera/sensor actions never touch the physical world (no
        # locks, gates, elevators), so they run under the same no-approval
        # policy as cctv_activate above rather than requiring a supervisor.
        "cctv_deactivate": {
            "approval_level": None,
            "auto_execute": True,
            "requires_active_incident": False,
